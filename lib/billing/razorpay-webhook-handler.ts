...
// PHASE-8: Handle PENDING subscription activation on payment
// When a payment is captured, check if there's a PENDING subscription waiting for this payment
async function activatePendingSubscription(admin: any, paymentId: string): Promise<boolean> {
  try {
    // Find PENDING subscription with matching pending_payment_id
    const { data: pendingSub, error: findError } = await admin
      .from('company_subscriptions')
      .select('id, company_id, plan_id, razorpay_subscription_id')
      .eq('status', 'PENDING')
      .eq('pending_payment_id', paymentId)
      .maybeSingle();

    if (findError) {
      console.error('PHASE-8: Error finding PENDING subscription:', findError);
      return false;
    }

    if (!pendingSub) {
      // No PENDING subscription found for this payment - this is normal for add-on payments
      return false;
    }

    console.log('PHASE-8: Activating PENDING subscription:', pendingSub.id);
    ...
    if (updateError) {
      console.error('PHASE-8: Error activating PENDING subscription:', updateError);
      return false;
    }

    // Also update the razorpay_subscription_id if not set
    if (!pendingSub.razorpay_subscription_id) {
      // Get the Razorpay subscription ID from the order or create one
      // For now, just update the status in companies table
      await admin
        .from('companies')
        .update({
        ...
        .eq('id', pendingSub.company_id);
    }

    // Write audit log
    await writeAuditLog({
      companyId: pendingSub.company_id,
      actor: 'system',
      action: 'subscription_activated_from_pending',
      status: 'success',
      integrationSystem: 'razorpay',
      metadata: {
    ...
    console.log('PHASE-8: Successfully activated PENDING subscription:', pendingSub.id);
    return true;
  } catch (error) {
    console.error('PHASE-8: Unexpected error activating PENDING subscription:', error);
    return false;
  }
}
