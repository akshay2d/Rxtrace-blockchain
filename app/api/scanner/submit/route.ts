...
export async function POST(req: Request) {
  try {
    ...
    const { role, company_id: tokenCompanyId, handset_id } = decoded;
    ...
    const companyId = handset.company_id;
    if (!companyId) {
      return NextResponse.json({ success: false, error: 'Invalid handset company mapping' }, { status: 403 });
    }

    // Prevent tenant spoofing: token company claim (if present) must match DB company.
